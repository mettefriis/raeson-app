// ─── Shared primitives used across all Raeson pages ────────────────────────
const { useState, useEffect, useRef, useLayoutEffect, useMemo } = React;
const { motion, useScroll, useTransform, useSpring, AnimatePresence, useMotionValue, useMotionValueEvent } = window.Motion;
...
function Grain({ enabled = true, opacity, isDark = false }) {
  ...
  const op = opacity ?? (isDark ? 0.14 : 0.22);
  return (
    <>
      {/* grain layer */}
      <div style={{
...
// ── Nav ───────────────────────────────────────────────────────────────────
function Nav({ C, current = 'home', hideWordmark = false }) {
  const links = [
    { label: 'Platform', href: 'Platform.html', key: 'platform' },
    ...
    <header style={{
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', pointerEvents: 'auto' }}>
        ...
        <a href="Demo.html" style={{
          fontSize: 13, fontWeight: 500, color: C.bg, textDecoration: 'none',
          padding: '9px 16px',
          background: C.text,
          borderRadius: 9999,
  ...
  return (
    <footer style={{
      ...
      <div style={{
        ...
        {[
          ['Product', [['Platform', 'Platform.html'], ['Pricing', 'Pricing.html'], ['Demo', 'Demo.html']]],
          ['Studio', [['About', 'About.html'], ['Method', 'About.html'], ['Contact', 'Contact.html']]],
          ['Resources', [['Journal', 'Journal.html'], ['Documentation', '#'], ['Changelog', '#']]],
          ['Legal', [['Privacy', '#'], ['Terms', '#'], ['Security', '#']]],
        ].map(([h, links]) => (
          <div key={h}>
            <p style={{ fontSize: 13, fontWeight: 500, color: C.dim, margin: '0 0 14px' }}>{h}</p>
            {links.map(([label, href]) => (
              <a key={label} href={href} style={{ display: 'block', fontSize: 13, color: C.dim, textDecoration: 'none', marginBottom: 8, transition: 'color 0.15s' }}
                onMouseOver={e => e.currentTarget.style.color = C.text}
...
Object.assign(window, {
  DARK, LIGHT, NIGHT, DEFAULT_TWEAKS, GRAIN_URL, EASE,
