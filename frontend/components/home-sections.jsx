...
const { useRef: useRefH } = React;
const { motion: motionH } = window.Motion;

const EASE_H = [0.16, 1, 0.3, 1];

// ── Hero ─────────────────────────────────────────────────────────────────
function CinematicHero({ C }) {
  return (
    <section style={{
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', width: '100%', padding: '0 48px', textAlign: 'center' }}>
        ...
        <motionH.div key="sub"
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <PrimaryBtn C={C} href="Demo.html">Run the demo</PrimaryBtn>
            <GhostBtn C={C} href="Platform.html">How it works</GhostBtn>
          </div>
        </motionH.div>
...
function PrimaryBtn({ C, href, children }) {
  return (
    <a href={href} style={{
      ...
      borderRadius: 999, fontSize: 13, fontFamily: 'var(--font-display)', fontWeight: 500, textDecoration: 'none',
...
// ── Section helpers ──────────────────────────────────────────────────────
function FadeUp({ children, delay = 0, y = 20, style, duration = 0.7 }) {
  return (
    <motionH.div initial={{ opacity: 0, y }} whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: '-60px' }}
      transition={{ duration, ease: EASE_H, delay }} style={style}>{children}</motionH.div>
  );
}
    ...
    <section style={{ position: 'relative', zIndex: 2, borderTop: `1px solid ${C.border}`, padding: '240px 48px', background: C.bg }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
        ...
        <FadeUp delay={0.08}>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <PrimaryBtn C={C} href="Demo.html">Run the demo</PrimaryBtn>
            <GhostBtn C={C} href="Contact.html">Book a walkthrough</GhostBtn>
          </div>
        </FadeUp>
      </div>
    </section>
...
Object.assign(window, { CinematicHero, ProblemSection, ProductTease, HowItWorks, Practices, CtaBand, PreviewFrame, FadeUp });
